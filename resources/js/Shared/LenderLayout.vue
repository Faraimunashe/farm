        ...
        <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200">
          <div class="flex items-center space-x-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-gray-500 truncate">{{ role?.display_name || 'Lender2' }}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="bg-white shadow-sm border-b border-gray-200">
          <div class="flex items-center justify-between h-16 px-6">
            ...
            <div class="flex items-center space-x-4">
              <a href="/lender/profile" class="text-gray-600 hover:text-gray-900">
  ...
  export default {
    computed: {
      ...
      pageTitle() {
        ...
        if (path.startsWith('/lender/dashboard')) return 'Dashboard'
        if (path.startsWith('/lender/loans')) return 'Loan Management'
        if (path.startsWith('/lender/transactions')) return 'Transactions'
        if (path.startsWith('/lender/reports')) return 'Reports & Analytics'
        if (path.startsWith('/lender/profile')) return 'Profile & Settings'
        if (path.startsWith('/lender/documents')) return 'Documents'
        return 'Dashboard'
      },
      navigation() {
        return [
